...
func writeTempScript(t *testing.T, src string) string {
	t.Helper()
	// Create temp script in the current working directory, not t.TempDir(),
	// to satisfy the execution directory restriction in Load().
	cwd, _ := os.Getwd()
	f, err := os.CreateTemp(cwd, "script_*.py")
	if err != nil {
		t.Fatalf("writeTempScript: %v", err)
	}

	// Ensure __pycache__ generated by python is also cleaned up.
	t.Cleanup(func() {
		os.Remove(f.Name())
		os.RemoveAll(filepath.Join(cwd, "__pycache__"))
	})
	defer f.Close()
